...
def noisy_agent(_control, vehicle, agent=None):
    ...
    cur_time = time.time()
    is_noise_active = cur_time - agent.noise_start_time < agent.noise_active_duration
    Recorder.steer_noise = steer_noise if steer_noise != 0 else agent.steer
    Recorder.gas_noise = c.throttle
...
class World(object):
    ...
    def restart(self):
        ...
        # 데이터 저장용 전면 카메라 부착
        self.center_camera = FrontCamera(self.player, "center")
        self.center_camera.set_sensor()
        self.left_camera = FrontCamera(self.player, "left", yaw=-30)
        self.left_camera.set_sensor()
        self.right_camera = FrontCamera(self.player, "right", yaw=30)
        self.right_camera.set_sensor()
    ...
    def destroy(self):
        actors = [
            ...
            self.center_camera.sensor,
            self.right_camera.sensor,
            self.left_camera.sensor,
            self.player]
        for actor in actors:
...
class FrontCamera(object):
    def __init__(self, parent_actor, role_name, yaw=0):
        self.recording = False
        self.sensor = None
        ...
        Attachment = carla.AttachmentType
        # TODO 카메라 각도 설정
        self.camera_transform = (carla.Transform(carla.Location(x=1.0, z=2.0), carla.Rotation(yaw=yaw, pitch=-15.0)),
                                 Attachment.Rigid)

        world = self._parent.get_world()
        self.blueprint = world.get_blueprint_library().find('sensor.camera.rgb')
        self.blueprint.set_attribute('role_name', self.role_name)  # role_name 설정
        self.blueprint.set_attribute('sensor_tick', '0.1')
    ...
    def _parse_image(weak_self, image):
        ...
        # pts = np.array([[0, 0], [0, h], [160, h], [140, 100],
        #                [260, 100], [240, h], [w, h], [w, 0]], dtype=np.int32)
        # cv2.fillPoly(np_image, [pts], (0, 0, 0))

        # np_image = cv2.Canny(np_image, 20, 60)
        # np_image = cv2.cvtColor(np_image, cv2.COLOR_GRAY2RGB)

        if self.role_name == "center":
            Recorder.center_image = np_image  # Recorder 로 이미지 전송
        elif self.role_name == "left":
            Recorder.left_image = np_image  # Recorder 로 이미지 전송
        elif self.role_name == "right":
            Recorder.right_image = np_image  # Recorder 로 이미지 전송

        if self.recording:
            image = Image.fromarray(np_image)
            # image.save('output/%s_%.3f.png' % (self.role_name, Recorder.platform_time))
